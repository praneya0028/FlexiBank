...
let logoutTiming: number;
let transferTiming: number;
...
const startLogoutTimer = function () {
  const tick = function () {
    const mins = String(Math.trunc(logoutTimer / 60)).padStart(2, '0');
    const secs = String(logoutTimer % 60).padStart(2, '0');
    labelTimer.textContent = `${mins}:${secs}`;
    // Logout once we reach 0
    if (logoutTimer === 0) {
      clearInterval(logoutTiming);
  ...
  // Set time to 5 min
  let logoutTimer = 100; // Adjust the timer value as needed
  tick();
  const timing = setInterval(tick, 1000);
  return timing;
};

// Logout with logout button
...
btnLogin.addEventListener('click', function (e) {
  ...
  // if validation passed
    ...
    if (currentAccount && currentAccount.pin === Number(inputLoginPin.value)) {
      ...
      // Clear time if it started on another account
      if (logoutTiming) clearInterval(logoutTiming);

      // Start logout timer
      logoutTiming = startLogoutTimer();
    } else {
      // Incorrect PIN
      inputLoginUsername.classList.add('warning');
      inputLoginPin.classList.add('warning');
      warningText.textContent = 'Incorrect PIN!';
    }
  }
});
...
btnTransfer.addEventListener('click', function (e) {
  ...
  if (currentAccount && currentAccount.balance && receiverAcc) {
    const balance =  currentAccount.balance;

    if (amount > 0 && balance >= amount && receiverAcc.username !== currentAccount.username) {
      // Transfer
      ...
      clearInterval(transferTiming);
      transferTiming = startLogoutTimer();
    }
  }
});
...
btnClose.addEventListener('click', function (e) {
  ...
  if (currentAccount) {
    ...
    if (username && inputCloseUsername.value === username && Number(inputClosePin.value) === currentAccount.pin) {
...
btnLoan.addEventListener('click', function (e) {
  ...
  if (inputLoanAmountValue) {
    ...
    if (movements && amount > 0 && currentAccount.movements.some(mov => mov >= amount / 10)) {
      // Set timer for loan
      ...
      setTimeout(function () {
        ...
        // Reset timer on action
        clearInterval(logoutTiming);
        const newTiming = startLogoutTimer(); // Start new timer
      }, 2500);
    }
  }
